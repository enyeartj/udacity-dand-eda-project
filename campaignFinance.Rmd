...
# Note: not setting echo=FALSE because there is *some* code I want to show
...
# Overall Stats:
summary(pcf.by_cand_nm)
subset(pcf.by_cand_nm, n == 1)
subset(pcf.by_cand_nm, total_amt == min(pcf.by_cand_nm$total_amt))
subset(pcf.by_cand_nm, total_amt == max(pcf.by_cand_nm$total_amt))
```

We can see that the median number of contributions for a candidate is 53. There
are two candidates tied for the minimum number of contributions: 1, and Bernie
Sanders has the most contributions with 4,262.

As far as dollar amount, the two candidates with the lowest total amount had
only $250 (and were also the same two candidates with only one contribution),
the median was $27,075, and the max amount was $832,643, which went to
Hillary Clinton.
...
The Democratic Party had 7,259 contributions for a total dollar amount of
$1,184,627 with a median contribution amount of $35.

The Republican Party had 7,766 contributions for a total dollar amount of
$1,873,199 with a median contribution amount of $50.
...
Let's start off with a look at contribution amounts.
...
select(pcf.by_cand_pt, cand_pt, median_inc)
```

Here we see that the median income of Republican Party contributors is
$52,633.70 and the median income for Democratic Party contributors is
$52,504.94.
...
select(pcf.by_cand_pt, cand_pt, median_amt_per_inc)
```

This is a bit more interesting. It looks like Republican supporters may be more
willing to spend a larger portion (about 0.1%) of their income on campaign
contributions than Democrats (about 0.07%). Let's look at the box plots.
...
Next, let's do a scatter plot of contribution amounts vs. median income.
...
There isn't much correlation between income and contribution amount.
The Pearson's correlation coefficient between these two variables is only 0.02.
Let's see if counties with a larger population are more likely to have more
contributions.
...
The Pearson's correlation coefficient for these variables is 0.98, which is
almost is close to being perfectly correlated.
Let's also look at total amount of contributions vs population.
...
select(ni, county, county_income)
summary(ni$county_income)
```

Median income by county ranges from $31,530 in Esmeralda County to $76,560 in
Lander County. The mean of the median incomes is $53,420. It is kind of
surprising that Clark County (where Las Vegas is) is not at the top of the list!
...
select(np, county, jul2015county_pop_est)
summary(np$jul2015county_pop_est)
```

The least populated county is the same as the one with the lowest median income,
Esmaralda County, with a population of only 829 people. The county with the
largest population by a wide margin is Clark County (one again, where Las Vegas
is) with a population of 2,115,000 people. The mean county population is 170,000
people and the median is 17,020 people.
...
options(dplyr.print_max = 16)
pcf.by_county_amt_ratio
```

The county that contributed the biggest percentage of total dollars to the
Democratic Party was Storey County with 97% of the contribution money going to
Democrats.
The county that contributed the biggest percentage of total dollars to the
Republican Party was a three-way tie between Esmeralda County, Lander County,
and Lyon County with 100% of the contribution money going to
Republicans.
...
pcf.by_county_n_ratio
options(dplyr.print_max = 10)
```

Perhaps unsurprisingly, the counties with the highest *number* of contributions
to Democrats and Republicans are also the counties with the highest *amount* of
contributions.
...
For comparison, the overall median contribution amount was $50, the mean was
$203.50, and the max was $10,800.
...
more likely to contribute to Democratic Party candidates. For example, Clark
County and Washoe County have the largest populations in Nevada and contributed
50.2% and 53.7% respectively to the Democratic Party, while Esmeralda County 
has the smallest populations and contributed 100% to the Republican Party. (The
county with the second highest population, Eureka County, made no contributions
to either party.)
...
Future work that could be performed on this data set could be to take a look at
the numbers for each party leading up to each presidential election. For
example, for this election, the Republicans have a higher number contributions
and a higher total amount of contributions in Nevada compared to the Democrats,
but what did the numbers look like in 2004, 2008, and 2012? Also, for the few
candidates that run in multiple election years, it would be interesting to see
if they did any better the second time around. Along these same lines, it would
also be interesting to see if different occupations support different parties in
different elections or if they tend to stick to the same parties.
